...
def sample_from_partitions(partitions, nb_rows, rng):
    """
    Sample values from csvw-safe:public.partitions
    Handles categorical and continuous partitions.
    """
    if not partitions:
        return pd.Series([pd.NA] * nb_rows)

    first = partitions[0].get("csvw-safe:predicate", {})

    # CATEGORICAL
    if "partitionValue" in first:
        values = [p["csvw-safe:predicate"]["partitionValue"] for p in partitions]
        return pd.Series(rng.choice(values, size=nb_rows))

    # CONTINUOUS
    if "lowerBound" in first:
        samples = []
